...
const initializeSocket = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Join a specific ticket room for real-time comments
    ...
    socket.on('comment:typing', ({ ticketId }) => {
      socket.to(`ticket:${ticketId}`).emit('comment:user_typing', {
        userId: user._id,
        userName: user.name,
        ticketId,
      });
    ...
    socket.on('comment:stop_typing', ({ ticketId }) => {
      socket.to(`ticket:${ticketId}`).emit('comment:user_stop_typing', {
        userId: user._id,
        ticketId,
      });
    });
